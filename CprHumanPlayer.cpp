...
// #define REPEAT_INPUT
  	// activate when generating *.out for judgegirl
...
void CprHumanPlayer::readCommands() {
	...
	while (!b && getline(cin, _command)) {
		if (!_command.empty() && _command.back() == '\r') {
			// to deal with redirected input in linux environment
			_command.pop_back();
		}
...
void CprHumanPlayer::parseHandCmd() {
	...
	if (!(in >> handNum) || handNum > 3 || handNum <= 0) {
		...
		if ( !(in >> cards[0] && in >> cards[1] && in >> cards[2]) ||
			...
			cout << "Command error. Try 'help' to continue." << endl;
		}
	} else {
